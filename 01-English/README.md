# 01 · English Language and Literature (Code 184)

- **Theory:** 80 marks · **Internal assessment:** 20 marks · **Time:** 3 hours
- **Prescribed books:** *First Flight* (main reader), *Footprints Without Feet*
  (supplementary reader), *Words and Expressions II* (workbook)

The paper has three parts — **Reading Skills**, **Writing & Grammar**, and
**Literature**. Roughly a third of the marks are not chapter-based at all, so
`03-Notes/Grammar-and-Writing/` matters as much as the chapter folders.

## Notes are split four ways

```
03-Notes/
├── First-Flight-Prose/        9 chapters
├── First-Flight-Poems/       10 poems
├── Footprints-Without-Feet/   9 chapters
└── Grammar-and-Writing/
    ├── 01-Grammar/
    └── 02-Writing-Skills/     letter, article, analytical paragraph
```

`04-NCERT-Solutions/` has the same three book-level folders; drop solved
exercise answers in there.

## First Flight — Prose

| # | Chapter |
|---|---|
| 1 | A Letter to God |
| 2 | Nelson Mandela: Long Walk to Freedom |
| 3 | Two Stories about Flying |
| 4 | From the Diary of Anne Frank |
| 5 | Glimpses of India |
| 6 | Mijbil the Otter |
| 7 | Madam Rides the Bus |
| 8 | The Sermon at Benares |
| 9 | The Proposal |

## First Flight — Poems

| # | Poem |
|---|---|
| 1 | Dust of Snow |
| 2 | Fire and Ice |
| 3 | A Tiger in the Zoo |
| 4 | How to Tell Wild Animals |
| 5 | The Ball Poem |
| 6 | Amanda! |
| 7 | The Trees |
| 8 | Fog |
| 9 | The Tale of Custard the Dragon |
| 10 | For Anne Gregory |

## Footprints Without Feet — Supplementary Reader

| # | Chapter |
|---|---|
| 1 | A Triumph of Surgery |
| 2 | The Thief's Story |
| 3 | The Midnight Visitor |
| 4 | A Question of Trust |
| 5 | Footprints without Feet |
| 6 | The Making of a Scientist |
| 7 | The Necklace |
| 8 | Bholi |
| 9 | The Book That Saved the Earth |

## Removed by the rationalisation

These appear in older editions and old question banks — they are **not** in the
current paper: *The Hundred Dresses I*, *The Hundred Dresses II*, the poem
*Animals*, and *The Hack Driver* from Footprints Without Feet.

## What to keep per chapter

For literature the marks come from reference-to-context extracts and short/long
answers, so a chapter folder is most useful with:

- `summary.md` — 150 words, in your own words
- `characters-themes.md` — who, what they represent, the central idea
- `extracts.md` — 4–5 likely passages with the answers you would write
- `word-meanings.md` — difficult words with meanings, for the unseen/grammar sections
